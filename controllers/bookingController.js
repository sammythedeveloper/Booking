...
exports.showBookingForm = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "An error occurred while retrieving the booking form" });
  }
};
...
exports.createBooking = async (req, res) => {
  try {
    ...
    // Create a new booking
    const booking = new Booking({
      user: req.user._id, // 👈 RIGHT HERE: Grab the active session user's ID
      firstName,
      lastName,
      email,
      phone,
    ...
    res
      .status(500)
      .json({ message: "An error occurred while creating the booking", error });
  }
};
...
exports.showConfirmationPage = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send("An error occurred while displaying the confirmation page");
  }
};
